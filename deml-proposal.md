...
今回、AutoDaiwarer のデータ形式を「台割記述言語（DEML: Daiwari Editorial Markup/Language）」と呼ぶことを提案します。
...
- 保存時の拡張子は、従来どおり `.txt` でも、提案名に合わせて `.deml` でも構いません。
- 実体はどちらもプレーンテキストです。
...
I propose calling this text format **DEML**, the *Daiwari Editorial Description Language*.
...
- Files may be saved with either `.txt` (conventional) or `.deml` (proposal-oriented) extension.
